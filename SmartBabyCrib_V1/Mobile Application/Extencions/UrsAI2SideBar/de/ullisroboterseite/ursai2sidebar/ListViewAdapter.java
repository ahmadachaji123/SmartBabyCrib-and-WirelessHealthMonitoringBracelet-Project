...
public class ListViewAdapter extends BaseAdapter {
   ...
   public ListViewAdapter(Context context, ItemDefinitionList itemDefinitions, SideBar sideBar, int barWidth) {
      this.context = context;
      this.itemDefinitions = itemDefinitions;
      this.sideBar = sideBar;

      for (int i = 0; i < itemDefinitions.size(); i++) {
         SideBarItemView v = new SideBarItemView(context, itemDefinitions.get(i), barWidth);
   ...
   public View getView(int position, View convertView, ViewGroup parent) {
      SideBarItemView v = views.get(position);
